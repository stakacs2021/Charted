"""
Maritime Identification Digits (MID) -> country lookup.
MMSI first 3 digits identify vessel's flag state.
Source: ITU Table of Maritime Identification Digits (subset of common MIDs).
"""
# Common MIDs: mid -> (country_name, iso2)
MMSI_MID_TO_COUNTRY: dict[int, tuple[str, str]] = {
    201: ("Albania", "AL"),
    202: ("Andorra", "AD"),
    203: ("Austria", "AT"),
    204: ("Azores", "PT"),
...
def mmsi_to_country(mmsi: str) -> tuple[str, str] | None:
    """Return (country_name, iso2) for ship MMSI, or None if unknown. Ships: first 3 digits = MID."""
    if not mmsi or len(mmsi) < 3:
        return None
    try:
        mid = int(mmsi[:3])
        return MMSI_MID_TO_COUNTRY.get(mid)
    except ValueError:
        return None
